...
export default function setSlide(clsName) {
  ...
  // 슬라이드 만큼 모두 호출하기!
  banBox.forEach((ele) => {
    // 하위 슬라이드 선택 요소(드래그 대상요소인 슬라이드)
    // let subSlide = mFn.qsEl(ele, ".slide");
    // -> slideFn 함수에서 하위 .slide를 수집하고 있음!
    // 따로 보낼 필요 없음!!!
...
function slideFn(selEl) {
 ...
 // 슬라이드 오른쪽 버튼 클릭시 왼쪽 방향 이동함수 ////////////
  function rightSlide() {
    ...
    setTimeout(() => {
      ...
      // 4.slide left값 -220% -> 최종 left 값은 px로!
 ...
 function leftSlide(leftVal="-330%") {
    // 슬라이드 왼쪽 버튼 클릭시 오른쪽 방향 이동함수 ////////////
    // 드래그 이동시엔 left 값을 -330%가 아닌
    // 드래그가 이동된 값을 적용한 left 값을 적용한다!
    // 함수 전달 변수를 leftVal = "330%"로 기본 입력값 처리하면
    // 함수 호출 시 전달 값이 없는 경우엔 기본 값으로 처리하고
    // 함수 호출 시 전달 값이 있으면 그 전달될 값으로 처리한다!
    // 이것을 함수 전달 변수 기본 입력 값 처리라고 한다!
    console.log('왼쪽 버튼 이동 left값:',leftVal);
    // leftVal - li 앞에 이동 시 left 값 설정 변수
    // 1. 슬라이드 li 새로 읽기
  ...
  ************************************************************/
  ...
  function slideAuto() {
    autoI = setInterval(() => {
      ...
      // 블릿변경함수호출(오른쪽은 1)
      chgIndic(1);
      // 중앙 슬라이드 클래스 on 넣기 함수 호출
      addOnSlide(3);
      // -> 오른쪽 버튼 (왼쪽 이동)이므로 2를 보냄!
  ...
  ************************************************************/
  ...
  // 기준 위치값 변수에 할당! -> originalValue 변수
  let leftVal = originalValue;
  // 왼쪽으로 이동할 기준 값(기준 위치 값*1.1)
  let valFirst = leftVal*1.1;
  // 오른쪽으로 이동할 기준 값(기준 위치 값*0.9)
  ...
  const dMove = (e) => {
    ...
    // e - 이벤트 객체 전달 변수
    // 드래그 상태는 dragSts 값이 true인 경우에만 허용!
    if (dragSts) {
      // 0. 자동넘김 멈춤함수 호출하기
      // clearAuto();
      ...
      // - 모바일용 터치 스크린 터치 위치는 touches[0].screenX, touches[0].screenY
  ...
  // (4) 첫번째 위치 포인트 세팅 함수 : firstX, firstY 값 세팅
  const firstPoint = (e) => {
  ...
  // -> 왜 필요하지? 이동 후 결과 위치를 저장하여 다음 드래그 이동시 그 결과를 중첩하여 반영하기 위해 필요함!!!
  const lastPoint = () => {
  ...
  const moveDragSlide = () => {
    ...
    // 대상의 left 값 찍기(xp단위를 parseInt()로 없애기!)
    ...
    // 드래그시 더해지는 마지막 위치값 lastX를
    // -220%의 left px 값으로 초기화 해준다!(숫자만!)
    lastX = originalValue;
    // -> 이것을 해야 오작동이 없다!!!
...
///////////////////////////////////////////////////////////////////////////
  ...
  mFn.addEvt(dtg, "mousedown", (e) => {
    ...
    // 첫번때 위치포인트 세팅!
    firstPoint(e);
    // 단독 할당되지 않고 내부 함수 호출로 연결되어 있으므로 이벤트 전달을 토스해줘야 한다!(전달변수 e)
  ...
  abtn.forEach(ele=>ele.addEventListener(
  ...
  mFn.addEvt(window,"resize",()=>{
    ...
    // 4. 왼쪽으로 이동할 기준 값(기준 위치 값*1.1)
    valFirst = leftVal*1.1;
    // 5. 오른쪽으로 이동할 기준 값(기준 위치 값*0.9)
