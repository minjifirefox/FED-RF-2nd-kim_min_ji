...
// [일반 함수를 생성자 함수로 만드는 가장 쉬운 방법]
// 1. 함수 이름을 대문자로 시작한다
// 2. 호출시 new 키워드를 사용하여 인스턴스를 생성한다!
// -> 일반 함수와 차이점은 개별적인 객체 인스턴스로 함수가
// 개별화 되기 때문에 운용상 독립적 프로세스를 확보하게 된다!
...
********************************************************/
  ...
  *********************************************************/
  ...
  // 터치시 위치값 변수 mPosStart 시작 위치 / mPosEnd 끝 위치
  let mPosStart = 0,
    mPosEnd = 0;
  ...
  function touchEndFn(e) {
    ...
    // 읽어와야 하므로 changedTouches[0]를 사용해야 함!
  
    // 2. 처음 터치 위치와 마지막 위치의 차 구하기
    let diffValue = mPosEnd - mPosStart;
    // 음수가 아랫방향 이동으로 맞추기 위해 (끝 값 - 첫 값)
  
    // 분석결과 :
    // 양수는 위에서 아래로 쓸어내림(위 페이지로 이동)
    // 음수는 아래에서 위로 쓸어올림(아래 페이지로 이동)
