...
// sort()는 기본 문자로 처리하므로 숫자는 내부함수로 빼기 연산 처리함!
// // console.log('숫자배열:',arrNumber2);
// // console.log('숫자배열 오름차순정렬 sort():',arrNumber2.sort());
// // console.log('숫자배열 오름차순정렬 sort((a,b)=>a-b):',arrNumber2.sort((a,b)=>a-b));
// // console.log('숫자배열 내림차순정렬 sort((a,b)=>a-b):',arrNumber2.sort((a,b)=>b-a));

// // console.log('문자배열:',arrString);
// // console.log('문자배열 오름차순 sort():',arrString.sort());
// // console.log('문자배열 내림차순 reverse():',arrString.reverse());
// // console.log('문자배열 오름차순정렬 sort((a,b)=>a-b):',arrString.sort((a,b)=>a-b));
...
// (2) map() 메서드 없이 배열값을 이미지 태그로 변환하여 코드 만들기 함수
const returnTag = (x) => {
  // x는 배열 전달변수
  // 1. 태그 저장용 변수
  let hcode = "";
...
-> join('') 빈문자열 결합을 사용하면 배열값이 깨끗하게 문자열 덩어리오 그대로 출력된다!
->>> 배열.map().join('') -> 배열 맵쪼잉~!
-> join()을 안쓰면 배열의 기본값이 콤마로 연결되어 할당되는데 콤마를 없애려면 반드시 join()을 써야함!
// console.log('원본 배열:',arrNumber2);
**************************************************************************************************************/
// console.log('원본 배열로 태그 작성:',
arrNumber2.map((v) => `<숫자>${v}</숫자>`);

// console.log('원본 배열로 태그 작성한 배열을 문자열로 변경하기:',
arrNumber2.map((v) => `<숫자>${v}</숫자>`).join("💘");

// console.log('원본 배열로 데이터 작성:',
arrNumber2.map((v, i) => `💕등록순번${i + 1}:${v}포인트`);
...
// (5) 정렬 기준에 선택박스 변경 이벤트 발생시 정렬 변경하기(오름차순/내림차순)
// (5-1) 대상 : #sel 선택박스
...
function changeSort(e, arrObj) {
  ...
  // arrObj 배열전달변수 -> 원본 배열을 담음(주소복사됨!)
  // 원본 배열을 보존키 위해 깊은 복사를 함!
  // 배열값이 일반 배열값이므로 스프레드 연산자 사용!
  arrObj = [...arrObj];
  // -> 다시 새로운 배열로 값이 복사됨!!!

  // 1. 선택옵션값 읽어오기
  let optVal = e.currentTarget.value;
  // 추가 : 이벤트 발생 요소(선택박스)의 아이디 읽어오기
  ...
  if (optVal == 1) {
    ...
    // ((해석)) 앞 값 뒷 값 같으면 0, 뒷 값이 크면 -1, 앞 값이 크면 1
    // 즉, 앞 값이 크면 자리를 바꿔서 유지하므로 오름차순 정렬

    // ((공통 정렬 처리하기))
    // 문자든 숫자든 sort()메서드의 내부적 처리에서 앞뒤 문자가 같으면 0, 뒷 문자가 크면 -1, 뒷 문자가 작으면 1로 리턴 값을 처리하면 된다!
    // -> [숫자 시그널] : 0 아무것도 안함, 1 바꿔서 유지, -1 그대로 유지
    // -> [내부적 처리란?] 문자일 경우 '가'>'나' 1로 처리할 경우
    // '나','가'로 순서를 바꿔서 처리함(내림차순)
    // 즉, 문자열도 순서대로 글자 알파벳, 가나다라 순 등 특정 순서 기준이 브라우저에 구현되어 있음!

    // 빼기 처리는 문자 등 기타 데이터는 처리불가함!
    // sort() 빼기연산처리 : 앞 수 - 뒷 수
    // (양수 결과일 경우 순서 바꾸기 함!)
    // arrObj.sort((a,b)=>a-b);
  } //// if ////
  ...
  // 2-2. 내림차순 : 값 2
    ...
    arrObj.sort((a, b) => (a == b ? 0 : a < b ? 1 : -1));
    // ((해석)) 앞 값 뒷 값 같으면 0, 뒷 값이 크면 -1, 앞 값이 크면 1
    // 즉, 뒷 값이 크면 자리를 바꿔서 유지하므로 내림차순 정렬
  ...
  // ((주의!!!))
  // 원본 배열을 정렬 후엔 원본 배열은 없어진다!!! ㅜ0ㅜ
  // 원본 배열을 다른 변수에 할당 후 다른 변수를 정렬 후엔 여전히 원본 배열은 없어진다!!! ㅜ0ㅜ
  ...
  // 원본배열확인하기
  // console.log('정렬 후 원본 배열:',selId == "sel" ? arrNumber : arrString);
} //////// changeSort 함수 ////////////////////////////
...
mFn.addEvt(sel3,"change",()=>{
    ...
    // 이것을 value에 할당하면 선택박스 값이 첫번째로 변경된다!
    sel3.value = "0";
}); //////////////////// change 이벤트 함수 ///////////////////////
