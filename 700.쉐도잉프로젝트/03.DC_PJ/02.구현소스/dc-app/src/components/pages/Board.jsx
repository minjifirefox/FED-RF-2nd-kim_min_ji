...
const PagingList = ({
  ...
  ****************************************************/
  ...
  // 계산된 시작 값, 한계 값을 기준으로 코드를 생성
  ////// [1] for : 페이징 리스트 출력 시작 /////
  for (let i = initNum; i < limitNum; i++) {
    // 전체 페이징 번호를 만드는 i가 페이징 전체 개수보다
    // 클 경우 나가야함!
...

  {
    ...
    pgCode.unshift(
      ...
      // key값이 있어야 함! 단, 중복되면 안됨!
      // 중복 안되는 수인 마이너스로 세팅한다!
      <Fragment key={-2}>
        &nbsp;&nbsp;
        <a 
          href="#"
  ...
  const goPaging = (dir, opt) => {
    ...
    // 3. 새로운 페이징의 페이징 구역의 첫번째 페이지 번호 업데이트하기
    // -> 항상 이전 블록의 마지막 번호 +1이 다음 페이지 첫번호임!
    // 이동할 페이지 번호
    let landingPage = ((pgPgNum.current-1)*pgPgSize)+1;
    console.log("도착페이지번호:",landingPage);
    // 페이지 번호 상태변수 업데이트로 전체 리렌더링!!!
