...
function slideFn(selEl) {
  ...
  **********************************/
  ...
  const fixedPosition = () => {
    ...
    // (1) 현재 리스트 li 수집하기
    let currList = mFn.qsaEl(dtg,"li");
    // (2) 리스트 한 개 당 크기(li 가로크기)
    let oneSize = currList[0].offsetWidth;
    // (3) 한개 li 크기로 현재 left 위치 크기를 나누어서
    // 소수점 아래 결과는 반올림 해준다! -> 특정 위치로 이동함!
