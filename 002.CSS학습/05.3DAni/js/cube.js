...
/* 
    ...
    3. 다시 "멈춰!"버튼 클릭시 돌고 있던 큐브의 애니메이션 설정 상태를 변경하여 멈추게 한다(버튼은 다시 "돌아!"로 변경)
*/
...
btngo.onclick = function(){
    // 1. 함수호출확인(this는 버튼 자신)
    console.log('나야나!',this.innerText);

    // 2. 변경대상 : .cube -> cube 변수

    // 3. 변경내용 : 큐브에 클래스 on을 없으면 넣고 있으면 제거한다 -> 미리 세팅된 애니 작동/멈춤 됨
    cube.classList.toggle('on');

        // 큐브 거리 변경 변수세팅은
        // html 요소에 클래스 on을 넣기/빼기하면 적용된다!
        document.querySelector('html')
        .classList.toggle('on');
        
    // classList는 요소의 클래스만 전문적으로 다뤄주는 JS 내장 객체다!
