...
function IntroCar({ brand , modelNum }){
    // 전달 속성 값 : brand - 자동차 분류명, modelNum - 배열순번
    // props는 컴포넌트 호출시 속성 값을 객체로 전달하는 것을 받아주는 변수다!(변수명은 자유!)
    // 사용법 : props.속성명
    // 주의 : 속성명은 컴포넌트를 호출하는 곳에서 정해진 이름을 똑같이 사용해야 함!
    // 명시적으로 변수명을 사용하려면 구조분해할당형식을 사용하면 된다! {변수명,변수명,...}
    // 구조분해할당으로 받을 경우의 장점은
    // 특정 속성명을 지정하는 효과가 있고 몇 개의 어떤 속성을 받는지 개발자가 명시적으로 알 수 있다
...
/* 
    [ 데이터 매칭하기 : 속성 세팅을 위한 기초 ]
    1. 자동차 분류명 : carInfo 데이터 변수의 객체의 속성명
    2. 상세 모델 배열 번호 : 선택 객체 속성 값 하위 배열 번호
...
function DetailCarInfo({brand,model,color,opt}){
    // info는 세부적인 모델 정보 객체가 들어온다!
    // 전달 속성은 model(모델명), color(차색), opt(css옵션)
    console.log("CSS옵션객체:",opt);

    // 객체값 추가는 (객체변수.속성명=값)
    opt.width = '600px';
    // 실제 CSS 변수형 속성명으로 사용해야 효과 있음
    ...
    return(
        <React.Fragment>
            ...
            style={opt}
            />
            {/* 리액트 style 속성의 값으로 객체를 CSS속성에 맞게 주면 인라인 코드로 CSS를 세팅할 수 있다! */}
        </React.Fragment>
    );
} /////////////////// DetailCarInfo 컴포넌트 ////////////////////////////
