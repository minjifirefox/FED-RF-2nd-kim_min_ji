...
function EventShow() {
  ...
  const showAladin = () => {
    ...
    // (2) 이미지출력
    ReactDOM.render(
    <MakeImg isrc="./images/ala4.jpg" ialt="알라딘" />,
    alaBox);
    // 컴포넌트 호출시 전달변수를 세팅하여 보내야 하는데
    // 만약 전달변수 이름이 잘못되었거나 보내주지 않으면
    // 컴포넌트에서 에러가 나지 않고 해당 항목을
    // 제외하여 표시하지 않는 특징이 있음!
    // 여기서 icss가 있으니 안보내주니 표시되지 않는다!
  ...
  const getLamp = () => {
    ...
    // 3. 0.5초 후 램프 이미지 중앙 이동하기
    setTimeout(() => {
      let lampImg =
      mFn.qsEl(lampBox,"img").style;
      // 수직방향이동
      lampImg.top = "310px";
      // 수평방향 중앙계산 이동
...
function MakeImg({isrc,ialt,icss,overFn,clickFn,itit,idName}){
  // 리턴 코드 : return 키워드 바로 뒤에 JSX 태그를 바로 이어쓰거나
...
function moveCar(eleName){ // eleName - 요소명
  ...
  // 2. 번갈아서 왔다갔다 움직이기
  tg.style.translate = 
  tg.style.translate == "150%"?"0":"150%";

  tg.style.scale = 
  tg.style.scale == "2"?"1":"2";

  tg.style.transition = "2s ease-in-out";
