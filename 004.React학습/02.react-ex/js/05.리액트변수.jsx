...
-> current를 써야하는 이유는 useRef 컴포넌트 객체에서 이 변수를
...
// [4] 컴포넌트 외부 일반 변수
// (1) 전역변수역할을 하지만 컴포넌트가 여러개 사용될 때
// 이 변수를 업데이트 하면 여러가지 프로그램이 꼬인다!
// 그러므로 절대! 절대! 절대! 변경되는 변수를 바깥 쪽에
// 쓰지 않는다! -> 리액트 철칙!!!
// (2) 단, 변경되지 않는 데이터 const 변수나 공유 함수 등은
// 컴포넌트 바깥에 코딩할 수 있다!
let what = "난뭐임?";
...
function MainComp() {
  ...
  // [1] 이름 상태관리 변수 :
  // (1) 변경된 값을 컴포넌트 소멸 전까지 유지됨
  // (2) 변수값 변경시 리랜더링 됨
  // (3) 값 변경시 반영은 useEffect 구역에서 확인됨!
  // -> 의미: 실제 돔에 반영할 때 변경 값이 업데이트 되는 시점임!
  const [stsName, setStsName] = React.useState("공유");
  
  // [2] 컴포넌트 내부 일반 변수
  // -> 컴포넌트가 리랜더링 될 때 다시 초기화 함!
  let varName = "김수현";
      ...
      <div className="wrap">
        <div>
          <h1>
            1. useState 출력
            <br />
            <b>나는 <span className="name">{stsName}</span> 좋아해!</b>
          </h1>
          <h1>
            2. let 출력
            <br />
            <b>나는 <span className="name">{varName}</span> 좋아해!</b>
          </h1>
          <h1>
            3. useRef 출력
            <br />
            <b>나는 <span className="name">{refName.current}</span> 좋아해!</b>
          </h1>
          <h1>
            4. 바깥 let 출력
            <br />
            <b>나는 <span className="name">{what}</span> 좋아해!</b>
          </h1>
        </div>
        <div>
