...
export default function 이야기() {
  ...
  // 산 정보는 배열이므로 순회하여 해당 데이터를 할당함
  const selMtInfo = mtInfo.find((v) => {
    if (v.이름 == 나야나.mtName) return true;
  });

  // 버튼 만들기에 사용할 산 이름 배열 만들기
  // 기존 산 정보 객체의 배열에서 산 이름 값만 모아서
  // 새로운 배열을 만든다!
  // 이것이 map() 메서드의 본래 사용법이다!
  const mtTotal = mtInfo.map((v) => v.이름);
  // console.log("산 이름만 새 배열:",mtTotal);
  ...
  return (
    <div style={나야나.mtBoxCss}>
      {/* 1. 산 이름 타이틀 */}
      <h1>{나야나.mtName != "후지산" && <MtIcon mtName={나야나.mtName} />} {나야나.mtName} {나야나.mtName != "후지산" && <MtIcon mtName={나야나.mtName} />}</h1>
      {/* 2. 산 이미지 */}
      <img
        src={selMtInfo.이미지}
        alt={selMtInfo.이름}
      ...
      {mtTotal.map(
        (v) =>
        // 현재 산 이름(mtName)이 아닌 배열값만 버튼 생성
          v != 나야나.mtName && (
            <button
              style={{
                padding: "15px",
